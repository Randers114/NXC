	...
	for (int face = 0; face < 6; face++)
	{
		for (int piece = 1; piece < 8; piece += 2)
		{
			if (myCube[face][piece] == 'x')
			{
				...
				TextOut(0, LCD_LINE1, "Coordinates: ");
                NumOut(0, LCD_LINE2, face);
                NumOut(0, LCD_LINE3, piece);
...
void DeterministicWhiteCross()
{
  ...
  /*  	for (int faceCheck = 0; faceCheck < 6; faceCheck++)
	  {
    // printf("face = %d\n",face);
	  for (int pieceCheck = 1; pieceCheck < 8; pieceCheck += 2)
	  {
		 if (myCube[faceCheck][pieceCheck] == 'w' && myCube[faceCheck][4] == 'w')
		 {
         	whiteFace = faceCheck;
         	whitePiece = pieceCheck;
         	FindComplementingEdge();

         	if (myCube[complementFace][complementPiece] == myCube[complementFace][4])
         	{
            	evaluateCorrect++;
         	}
		 }
